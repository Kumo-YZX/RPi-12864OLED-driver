...
class Oled(object):
    ...
    def displayAsc(self, ascStr='example', pos=[0,0], fontFile='fonts/ASC16', fontSize=16):
        pass

    def displayChn(self, chnStr=u'\u5317\u4eac', pos=[0,0], fontFile='fonts/HZK16', fontSize=16):
        quwei =[]
        ...
        for everyChar in quwei:
            ...
            fontModel =[]
            # model of every line
            for line in range(0,fontSize*fontSize/8,fontSize/8):
                # model of every column part
                lineModel =0
                for everyPart in range(fontSize/8):
                    lineModel =lineModel +ord(fontList[fontAddress+line+everyPart])*(2**(8*(fontSize/8-1-everyPart)))
